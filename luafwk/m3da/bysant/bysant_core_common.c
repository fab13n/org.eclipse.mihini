...
bs_class_t* lua_bs_toclassdef( lua_State *L, int narg) {
  ...
  if( named) {
    lua_tolstring( L, -1, &size); size ++; // size does not include null terminator
    blocksize += size;
  }
  ...
  for( i=1; i<=nfields; i++) {
    ...
    if( !named && lua_isstring( L, -1)) { // short form
      ...
      if( named) {
        ...
        luaL_checklstring( L, -1, &size); size ++;
        blocksize += size;
        lua_pop( L, 1);
      }
    }
  ...
  if( named) {
    const char *name;
    lua_getfield( L, narg, "name"); // narg=>classdef, ..., -1=>name
    name = luaL_checklstring( L, -1, &size); size ++;
    memcpy( endofblock, name, size);
    classdef->classname = endofblock;
    endofblock += size;
  }
  classdef->fields = endofblock;
