...
--
-- The first DB describes mountpoints: it associates the mountpoint's
-- `lpath` to the handler and the mounted `hpath`, separated by a
-- colon character `":"`.
--
-- The second DB describes the inverse relation: it associates a
...
--
-- The fourth DB lists every handler mounted under every node, directly
...
--------------------------------------------------------------------------------
-- Translate a logical node into an enumeration of every handler
