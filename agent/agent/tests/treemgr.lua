...
function t :test_get1()
    local a, b = treemgr.get('tests.x.table1.numbers.fortytwo')
    u.assert_equal(a, 42)
    u.assert_nil(b)
end
...
function t :test_get_inexistent_in_handler()
    local a, b = treemgr.get('tests.x.table1.whatever')
    u.assert_nil(a)
    u.assert_nil(b)
end
...
function t :test_get_inexistent_out_of_handler()
    local a, b = treemgr.get('whatever.whatever.whatever')
    u.assert_nil(a)
    u.assert_string(b)
end
...
function t :test_get_recursive_raw()
    local n_nil, n_children = treemgr.get('tests.x.table1.numbers')
    u.assert_nil(n_nil)
    assert_table_eq_no_order(n_children, {
        'tests.x.table1.numbers.thirties',
        'tests.x.table1.numbers.fortytwo',
    })
    n_nil, n_children = treemgr.get('tests.x.table1.numbers.thirties')
    u.assert_nil(n_nil)
    assert_table_eq_no_order(n_children, {
    ...
    local n
    n, n_nil = treemgr.get('tests.x.table1.numbers.fortytwo')
    u.assert_equal(n, 42)
    u.assert_nil(n_nil)
    n, n_nil = treemgr.get('tests.x.table1.numbers.thirties.thirtyone')
    u.assert_nil(n_nil)
    u.assert_equal(n, 31)
    n, n_nil = treemgr.get('tests.x.table1.numbers.thirties.thirtythree')
    u.assert_nil(n_nil)
    u.assert_equal(n, 33)
end
...
function t :test_twohandlers()
    local a, b = treemgr.get{'tests.x.table1.numbers.fortytwo', 'tests.y.table2.key'}
...
function t :test_notify1()
    ...
    local r1 = treemgr.register({'tests.z'}, hook_z)
    u.assert(r1)
    local r2 = treemgr.register({'tests.z.ramstore.d.e.e.p'}, hook_deep,
...
function t :test_doublenotif()
    ...
    local h1=u.assert(treemgr.register({'tests.w.ramstore2'}, seen_on_ramstore_w))
    local h2=u.assert(treemgr.register({'tests.z.ramstore'}, seen_on_ramstore_z))
